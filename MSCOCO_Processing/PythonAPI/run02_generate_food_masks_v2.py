...
if __name__ == '__main__':
    ...
    makeDirIfNotExists(pathToDir=dirOut, isCleanIfExists=True)
    #
    ...
    for ii, kk in enumerate(setAllFoodImgIds):
        ...
        for vvi in vv:
            ...
            tmsk[tmpMsk>0]=vvi
        #
        mskPercent = np.sum((tmsk>0))/float(np.prod(tmsk.shape))
        if mskPercent<0.2:
            print ('\n\n!!!! WARNING !!!! mask size too small: %0.3f [%s], skip...' % (mskPercent, fimg))
            continue
